...
public class CheckBox extends AbstractComponent {
    ...
    public void render() {
        ...
        if (Config.instance.guiGlow.getValBoolean() && selected) Render2DUtil.drawRoundedRect(x / 2.0, y / 2.0, (x + preferredHeight) / 2.0, (y + preferredHeight) / 2.0, hovered ? Config.instance.guiAstolfo.getValBoolean() ? renderer.astolfoColorToObj() : Window.SECONDARY_OUTLINE : Window.SECONDARY_FOREGROUND, Config.instance.glowBoxSize.getValDouble());
        if (Config.instance.guiBetterCheckBox.getValBoolean() && selected) renderer.drawCheckMark(x / 2f - 5, y / 2f - 1, preferredHeight, Color.WHITE.getRGB());
