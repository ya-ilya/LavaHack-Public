    ...
    public void render() {
        ...
        if (maxY > 0) {
            ...
            if (Config.instance.guiGlow.getValBoolean()) {
                Render2DUtil.drawRoundedRect((x + getWidth() - sliderWidth) / 2.0, (y + (getHeight() - sliderHeight) * (scrollOffset / (double) maxY)) / 2, (x + getWidth()) / 2.0,(y + (getHeight() - sliderHeight) * (scrollOffset / (double) maxY) + sliderHeight) / 2, Config.instance.guiAstolfo.getValBoolean() ? renderer.astolfoColorToObj() : Window.SECONDARY_OUTLINE, Config.instance.glowBoxSize.getValDouble());
            }
        }
    }
