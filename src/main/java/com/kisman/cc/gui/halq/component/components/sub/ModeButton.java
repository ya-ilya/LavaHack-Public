...
import com.kisman.cc.event.events.client.settings.SettingChangeEvent;
...
public class ModeButton extends Component {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (HalqGui.shadowCheckBox) {
            ...
            Render2DUtil.drawAbstract(new AbstractGradient(new Vec4d(new double[] {x, y + offset}, new double[] {x + width / 2.0, y + offset}, new double[] {x + width / 2.0, y + offset + HalqGui.height}, new double[] {x, y + offset + HalqGui.height}), ColorUtil.injectAlpha(HalqGui.backgroundColor, 1), HalqGui.getGradientColour(count).getColor()));
            Render2DUtil.drawAbstract(new AbstractGradient(new Vec4d(new double[] {x + width / 2.0, y + offset}, new double[] {x + width, y + offset}, new double[] {x + width, y + offset + HalqGui.height}, new double[] {x + width / 2.0, y + offset + HalqGui.height}), HalqGui.getGradientColour(count).getColor(), ColorUtil.injectAlpha(HalqGui.backgroundColor, 1)));
        ...
        if (isMouseOnButton(mouseX, mouseY) && button == 0) open = !open;
            ...
            for (int i = 0; i < values.length; i++) {
                ...
                if (mouseY >= offsetY && mouseY <= offsetY + HalqGui.height) {
                    ...
                    setting.setIndex(i);
                    Kisman.EVENT_BUS.post(new SettingChangeEvent.ModeSetting(setting));
                    break;
                }
                offsetY += HalqGui.height;
            }
