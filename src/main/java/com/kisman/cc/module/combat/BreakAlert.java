        ...
        if (renderTimer.passedMillis(displayShowDelay.getValLong())){
            for (int i = 0; i < blocksBeginBroken.size(); i++) {
               if (messageType.getValString().equalsIgnoreCase(MessageType.Display.name())) {
                    ...
                    CustomFontUtil.comfortaab72.drawCenteredStringWithShadow(TextFormatting.DARK_PURPLE + "Break Alert!", sr.getScaledWidth() / 2.0, sr.getScaledHeight() / 2.0 - CustomFontUtil.getFontHeight(CustomFontUtil.comfortaab72), -1);
                    CustomFontUtil.comfortaab55.drawCenteredStringWithShadow(TextFormatting.LIGHT_PURPLE + "Your surround blocks is mining!", sr.getScaledWidth() / 2.0, sr.getScaledHeight() / 2.0 + 5, -1);
                }
            }
